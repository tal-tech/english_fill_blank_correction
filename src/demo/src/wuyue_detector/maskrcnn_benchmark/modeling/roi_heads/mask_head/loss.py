...
def project_masks_on_boxes(segmentation_masks, proposals, discretization_size, maskiou_on):
    ...
    for segmentation_mask, proposal in zip(segmentation_masks, proposals):
        ...
        if maskiou_on:
            ...
            temp_list = []
            for poly_ in segmentation_mask_for_maskratio.polygons:
                poly_ = poly_.polygons[0]
                temp_list.append(poly_)
            ...
            rle_for_fullarea = mask_util.frPyObjects([p.numpy() for p in temp_list], y2-y1, x2-x1)
            full_area = torch.tensor(mask_util.area(rle_for_fullarea).sum().astype(float))

            temp_list = []
            for poly_ in cropped_mask.polygons:
                poly_ = poly_.polygons[0]
                temp_list.append(poly_)
            ...
            rle_for_box_area = mask_util.frPyObjects([p.numpy() for p in temp_list], proposal[3]-proposal[1], proposal[2]-proposal[0])
...
class MaskRCNNLossComputation(object):
    ...
    def __call__(self, proposals, mask_logits, targets):
        ...
        # accept empity tensors, so handle it separately
