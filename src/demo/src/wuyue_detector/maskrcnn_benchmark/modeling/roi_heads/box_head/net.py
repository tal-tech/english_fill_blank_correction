...
import maskrcnn_benchmark.modeling.roi_heads.box_head.nn as mynn
...
def bbox_transform(deltas, weights):
    ...
    return x1.view(-1), y1.view(-1), x2.view(-1), y2.view(-1), pred_h.view(-1), pred_w.view(-1)
...
def compute_ciou(output, target, bbox_inside_weights, bbox_outside_weights,
    ...
    x1, y1, x2, y2, h_pred, w_pred = bbox_transform(output, transform_weights)
    x1g, y1g, x2g, y2g , h_gt, w_gt = bbox_transform(target, transform_weights)
    ...
    #print('min h_pred : ', torch.min(h_pred))
...
def compute_giou(output, target, bbox_inside_weights, bbox_outside_weights,
    ...
    x1 = output[:,0::4].view(-1)
    y1 = output[:,1::4].view(-1)
    x2 = output[:,2::4].view(-1)
    y2 = output[:,3::4].view(-1)
   
    x1g = target[:,0::4].view(-1)
    y1g = target[:,1::4].view(-1)
    x2g = target[:,2::4].view(-1)
    y2g = target[:,3::4].view(-1)
